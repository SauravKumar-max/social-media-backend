...
module.exports = {
	...
	resolve: async (parent, { searchInput }, req) => {
		try {
			...
			const searchByName = users.filter(user => user.name.toLowerCase().includes(searchInput))
			const searchByUsername = users.filter(user => user.username.toLowerCase().includes(searchInput));
