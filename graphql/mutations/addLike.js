...
module.exports = {
	...
	resolve: async (parent, { postId, postUserId }, req) => {
		try {
			...
			const updatePost = userPost.posts.map(post => post._id == postId ? { ...post.toObject(), likes: { ...post.likes, count: post.likes.count + 1, likedUserId: [...post.likes.likedUserId, userId] } } : post);
